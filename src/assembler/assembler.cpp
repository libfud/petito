    ...
    uint16_t pc) -> Result<int32_t, AsmError>
{
    ...
    auto evaluate_sub_expr = [&](const SExpr& sub_expr) -> std::optional<AsmError> {
        ...
        {
            auto expr_result = std::get<Expr*>(sub_expr)->evaluate(symbol_map, pc);
            if (expr_result.is_err())
            {
                return expr_result.get_err();
            }
            rhs = expr_result.get_ok();
        }
        return {};
    };
    ...
    if (unary_op != std::nullopt)
    {
        ...
        acc = unary_expr(unary_op.value(), rhs.value(), pc);
        unary_op = {};
        rhs = {};
    }
    else
    ...
    while (idx < expression.size())
    {
        ...
        if (unary_op != std::nullopt)
        {
            ...
            rhs = unary_expr(unary_op.value(), rhs.value(), pc);
            unary_op = {};
        }
        if (rhs == std::nullopt)
        {
...
auto ArithmeticExpression::unary_expr(UnaryOperator op, uint16_t rhs, uint16_t pc) -> int32_t
{
    switch (op)
    {
...
auto ArithmeticExpression::add_expression(Expr* nested_expression) -> void
{
    ...
    expression.push_back(nested_expression);
}
...
auto ArithmeticExpression::make(Context* context) -> ExprResult
{
    ...
    if (context->LBRACKET())
    {
        if (context->arithmetic().size() == 0)
        {
            return ExprResult::err(ParseError::BadArithmetic);
        }
        for (auto* nested_expr : context->arithmetic())
        {
            auto res = make(nested_expr);
            if (res.is_err())
            {
                return res;
            }
            auto* expr_ptr = new Expr{res.get_ok()};
            expression.add_expression(expr_ptr);
        }
        if (!context->RBRACKET())
        {
            return ExprResult::err(ParseError::BadArithmetic);
    ...
    if (context->number())
    {
        auto result = handle_number(context, expression);
...
auto ArithmeticExpression::handle_number(
    Context* context,
    Expr& expression) -> std::optional<ParseError>
{
    auto is_err = expression.handle_number_context(context->number());
    if (is_err != std::nullopt)
    {
        return is_err;
    }

    for (auto& binary_expr_context : context->binary_expression())
    {
        is_err = expression.handle_binary_op(binary_expr_context->binary_op());
        if (is_err != std::nullopt)
        {
            return is_err;
        }
        is_err = expression.handle_number_context(binary_expr_context->number());
        if (is_err != std::nullopt)
        {
            return is_err;
        }
...
auto ArithmeticExpression::handle_number_context(NumberContext* ctx) -> std::optional<ParseError>
{
    if (ctx->unary_op())
    {
...
auto ArithmeticExpression::handle_unary_op(UnaryOpContext* ctx) -> std::optional<ParseError>
{
    ...
    {
        add_atom(Atom{UnaryOperator::Star});
    }
    else
    {
...
auto Instruction::make(asm6502Parser::LineContext* line) -> InstructionResult
{
    ...
    if (assign_rule != nullptr)
    {
        ...
        auto expr_result = ArithmeticExpression::make(assign_rule->arithmetic());
        if (expr_result.is_err())
        {
            return InstructionResult::err(AsmError::BadAssign);
        }
        ...
        return InstructionResult::ok(instruction);
    }
    ...
    if (instruction_rule != nullptr)
    {
        ...
        if (res != std::nullopt)
        {
            return InstructionResult::err(AsmError::BadParse);
        }
...
auto Instruction::has_label() const -> bool { return label != std::nullopt; }

auto Instruction::get_label() const -> const std::string&
{
    return label.value();
}

auto Instruction::size() const -> uint16_t
{
    switch (line_type)
    {
...
auto Instruction::complete_decode(
...
auto Instruction::handle_instruction_rule(
...
auto Instruction::handle_immediate_rule(
...
auto Instruction::handle_x_index_rule(
...
auto Instruction::handle_y_index_rule(
...
auto Instruction::handle_relative_rule(
...
auto Instruction::handle_jump_rule(
...
auto Instruction::handle_jsr_rule(
...
auto Instruction::check_mnemonic(const std::string& name, AddressMode mode) -> BuilderResult
{
...
auto Instruction::handle_arithmetic(
...
auto Instruction::validate() -> BuilderResult
{
    return {};
}
...
auto Assembler::from_file_name(std::string&& name) -> AssemblerResult
{
    ...
    {
        auto instruction_result = Instruction::make(lines[idx]);
        ...
        assembler.program.push_back(instruction);
