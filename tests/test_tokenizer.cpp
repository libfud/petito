...
TEST(TestToken, Pragma)
{
    for (const auto& pair : PRAGMA_MAP)
    {
        std::string input{std::format(".{}", pair.first)};
        auto result = Token::tokenize_line(input);
        ASSERT_TRUE(result.is_ok());
        auto tokens = result.get_ok();
        ASSERT_EQ(tokens.size(), 1);
        ...
        ASSERT_EQ(std::get<token::Directive>(token.get()).value, pair.second);
    }
}

TEST(TestToken, InvalidPragma)
{
    std::string input{"."};
    auto result = Token::tokenize_line(input);
    ASSERT_TRUE(result.is_err());
    ASSERT_EQ(result.get_err(), AsmError::InvalidPragma);
...
TEST(TestToken, Keyword)
{
    for (const auto& pair : KEYWORD_MAP)
    {
        ...
        ASSERT_EQ(std::get<token::Reserved>(token.get()).value, pair.second);
    }
}
...
TEST(TestToken, Composite2)
{
    const auto op_id = OpName::BEQ;
    ...
    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::MathOp);
    ASSERT_EQ(std::get<MathOp>(token.get()).value, ArithmeticOp::Subtract);
    count++;

    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::Byte);
    ASSERT_EQ(std::get<Byte>(token.get()).value, bytes[0]);
    count++;

    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::MathOp);
    ASSERT_EQ(std::get<MathOp>(token.get()).value, ArithmeticOp::Add);
    count++;

    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::Byte);
    ASSERT_EQ(std::get<Byte>(token.get()).value, bytes[1]);
    count++;

    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::MathOp);
    ASSERT_EQ(std::get<MathOp>(token.get()).value, ArithmeticOp::Multiply);
    count++;

    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::LBracket);
    count++;

    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::Byte);
    ASSERT_EQ(std::get<Byte>(token.get()).value, bytes[2]);
    count++;

    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::MathOp);
    ASSERT_EQ(std::get<MathOp>(token.get()).value, ArithmeticOp::Divide);
    count++;

    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::Byte);
    ASSERT_EQ(std::get<Byte>(token.get()).value, bytes[3]);
    count++;

    ASSERT_GT(tokens.size(), count);
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::RBracket);
    count++;
    ...
    token = tokens[count];
    ASSERT_EQ(token.token_type(), TokenType::Byte);
    ASSERT_EQ(std::get<Byte>(token.get()).value, bytes[4]);
    count++;
